...
export const PrePrimaryHome = () => {
  const [PrePrimary, setPrePrimary] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        "http://gosaviadvanceddentalclinic.com:8003/api/getallpreprimary"
      );
      setPrePrimary(response.data);
    };
    fetchData();
  }, []);
          ...
          {PrePrimary.map((user) => {
            return (
              <div className="row">
                <h4>
                  {user.heading}{" "}
                  <Link to={`/PrePrimary-edit/` + user._id}>
                ...
                <img
                  src={`http://gosaviadvanceddentalclinic.com:8003/images/${user.preprimaryimage}`}
                  alt="Hi Tech Image"
                />
                <br />
                <p dangerouslySetInnerHTML={{ __html: user.description2 }}></p>
                <h5>{user.title}</h5>
                <p dangerouslySetInnerHTML={{ __html: user.description3 }}></p>
              </div>
            );
          })}
        </div>
...
export const EditPrePrimary = () => {
  ...
  const [PrePrimaryUser, setPrePrimaryUser] = useState(users);
  const [PrePrimaryImage, setPrePrimaryImage] = useState(null);
  const { id } = useParams();
  const navigate = useNavigate();
    ...
    const fetchDirectorData = async () => {
      try {
        const response = await axios.get(`http://gosaviadvanceddentalclinic.com:8003/api/getonepreprimary/${id}`);
        setPrePrimaryUser(response.data);
        ...
        if (window.CKEDITOR) {
          ...
          if (window.CKEDITOR.instances.editor3) {
            window.CKEDITOR.instances.editor3.setData(response.data.description3 || "");
          }
        }
      } catch (error) {
    ...
    useEffect(() => {
      if (editorLoaded && window.CKEDITOR.instances.editor1) {
        window.CKEDITOR.instances.editor1.setData(PrePrimaryUser.description1 || "");
      }
      if (editorLoaded && window.CKEDITOR.instances.editor2) {
        window.CKEDITOR.instances.editor2.setData(PrePrimaryUser.description2 || "");
      }
      if (editorLoaded && window.CKEDITOR.instances.editor3) {
        window.CKEDITOR.instances.editor3.setData(PrePrimaryUser.description3 || "");
      }
    }, [PrePrimaryUser.description1, PrePrimaryUser.description2, PrePrimaryUser.description3, editorLoaded]);
  


  const changeHandler = (e) => {
    const { name, value } = e.target;
    setPrePrimaryUser({ ...PrePrimaryUser, [name]: value });
  };
  ...
  const submitForm = async (e) => {
    e.preventDefault();
       ...
       if (window.CKEDITOR.instances.editor1) {
        PrePrimaryUser.description1 = window.CKEDITOR.instances.editor1.getData();
      }
      if (window.CKEDITOR.instances.editor2) {
        PrePrimaryUser.description2 = window.CKEDITOR.instances.editor2.getData();
      }

      if (window.CKEDITOR.instances.editor3) {
        PrePrimaryUser.description3 = window.CKEDITOR.instances.editor3.getData();
      }

    const formData = new FormData();
    formData.append("heading", PrePrimaryUser.heading);
    formData.append("description1", PrePrimaryUser.description1);
    formData.append("description2", PrePrimaryUser.description2);
    formData.append("title", PrePrimaryUser.title);
    formData.append("description3", PrePrimaryUser.description3);
    if (PrePrimaryImage) formData.append("preprimaryimage", PrePrimaryImage);

    try {
      await axios.put(
        `http://gosaviadvanceddentalclinic.com:8003/api/updatePrePrimary/${id}`,
        formData
      );
      ...
      console.error("There was an error updating the PrePrimary!", error);
    }
  };
...
export const AddPrePrimary = () => {
  ...
  useEffect(() => {
    if (window.ClassicEditor && document.getElementById("description-editor")) {
      ...
      window.ClassicEditor.create(document.getElementById("description-editor"))
        .then((editor) => {
          editor.setData(PrePrimaryUser.description2 || "");
          editor.model.document.on("change:data", () => {
            const data = editor.getData();
            setPrePrimaryUser((prev) => ({ ...prev, description2: data }));
          });
  ...
  }, [PrePrimaryUser.description2]);

  const inputHandler = (e) => {
    const { name, value } = e.target;
    setPrePrimaryUser({ ...PrePrimaryUser, [name]: value });
    console.log(PrePrimaryUser);
  };
  ...
  const submitForm = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://gosaviadvanceddentalclinic.com:8003/api/createPrePrimary",
        formData,
        {
          headers: {
      ...
      <div className="row">
        ...
        <div className="col-lg-10 aboutuser">
          ...
          <form className="row" onSubmit={submitForm}>
            ...
            <div className="col-lg-12 col-md-12 col-sm-12 aboutinputform">
              <label htmlFor="preprimaryimage">PrePrimary Image</label>
              <input
                type="file"
                onChange={(e) => setPrePrimaryImage(e.target.files[0])}
              />
            </div>
            <div className="col-lg-12 col-md-12 col-sm-12 aboutinputform">
              ...
              <input
                ...
                defaultValue={PrePrimaryUser.description2}
              />
            </div>
