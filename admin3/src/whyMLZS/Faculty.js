  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        "http://gosaviadvanceddentalclinic.com:8003/api/getallfaculty"
      );
    ...
    <div className="container principal">
        ...
        <i className="fa-solid fa-plus"></i> Add</button>
          ...
          return (
                ...
                <h4>
                {user.heading}{" "}
                <Link to={`/Faculty-edit/` + user._id}>
                  <i className="fa-regular fa-pen-to-square"></i>
                </Link>
                <br />
              </h4>
              <img
                src={`http://gosaviadvanceddentalclinic.com:8003/images/${user.facultyimage}`}
                alt="Hi Tech Image"
              />
              <br />
...
export const EditFaculty = () => {
  ...
  const [FacultyUser, setFacultyUser] = useState(users);
  ...
  const fetchPedagogyData = async () => {
    try {
      const response = await axios.get(`http://gosaviadvanceddentalclinic.com:8003/api/getonefaculty/${id}`);
  ...
  useEffect(() => {
    if (editorLoaded && window.CKEDITOR.instances.editor) {
      window.CKEDITOR.instances.editor.setData(FacultyUser.description);
    }
  }, [FacultyUser.description, editorLoaded]);



  const changeHandler = (e) => {
    const { name, value } = e.target;
    setFacultyUser({ ...FacultyUser, [name]: value });
  };
  ...
  const submitForm = async (e) => {
    e.preventDefault();
     ...
     if (window.CKEDITOR && window.CKEDITOR.instances.editor) {
      FacultyUser.description = window.CKEDITOR.instances.editor.getData();
    }

    const formData = new FormData();
    formData.append("heading", FacultyUser.heading);
    formData.append("description", FacultyUser.description);
    if (FacultyImage) formData.append("facultyimage", FacultyImage);

    try {
      await axios.put(
        `http://gosaviadvanceddentalclinic.com:8003/api/updatefaculty/${id}`,
        formData
      );
        ...
        <form onSubmit={submitForm}>
          <div className="modal-content">
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                <label>Faculty Image <span style={{color:"red"}}>("Image must be at least Width-500 Height-400 pixels!")</span></label>
                <input
                  type="file"
                  className="form-control"
                  accept="image/*"
  ...
  useEffect(() => {
    if (window.ClassicEditor && document.getElementById("description-editor")) {
      ...
      window.ClassicEditor.create(document.getElementById("description-editor"))
        .then((editor) => {
          editor.setData(FacultyUser.description || "");
          editor.model.document.on("change:data", () => {
            const data = editor.getData();
            setFacultyUser((prev) => ({ ...prev, description: data }));
          });
  ...
  const submitForm = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://gosaviadvanceddentalclinic.com:8003/api/createfaculty",
        formData,
        {
          headers: {
          ...
          <form className="row" onSubmit={submitForm}>
            ...
            <div className="col-lg-12 col-md-12 col-sm-12 aboutinputform">
              ...
              <input
                ...
                defaultValue={FacultyUser.description}
              />
            </div>
